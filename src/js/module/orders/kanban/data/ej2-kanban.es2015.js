...
import { Dialog, Tooltip, Popup, createSpinner, showSpinner, hideSpinner } from './ej2-popups.es2015.js';
import { DataManager, Query, UrlAdaptor, Deferred, Predicate } from './ej2-data.es2015.js';
import { DropDownList } from './ej2-dropdowns.es2015.js';
import { TextBox, NumericTextBox, FormValidator } from './ej2-inputs.es2015.js';
import { Button } from './ej2-buttons.es2015.js';
        ...
        if (this.parent.enableVirtualization) {
            ...
            if (addIndex.length > 0) {
                if (this.parent.dataModule.isRemote()) {
                    ...
                    const visibleStartIndex = this.scrollStatus[columnKey].scrollDirection === 'down'
                                              ? ((addIndex[0] - 1) * this.scrollStatus[columnKey].singleIndexCardCount)
                                              : ((addIndex[0] - 1) * this.scrollStatus[columnKey].singleIndexCardCount) + (this.scrollStatus[columnKey].singleIndexCardCount * addIndex.length) - 1;

                    const visibleLength = this.scrollStatus[columnKey].scrollDirection === 'down'
                                          ? visibleStartIndex + (this.scrollStatus[columnKey].singleIndexCardCount * addIndex.length) :
                                          ((addIndex[0] - 1) * this.scrollStatus[columnKey].singleIndexCardCount);
...
export { Kanban, actionBegin, actionComplete, actionFailure, bottomSpace, cardClick, cardDoubleClick, cardRendered, cardSpace, contentReady, dataBinding, dataBound, dataReady, dataSourceChanged, dataStateChange, dialogClose, dialogOpen, drag, dragStart, dragStop, queryCellInfo, toggleWidth };
