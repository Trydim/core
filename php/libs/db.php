		...
		array_map(function ($col) use (&$result, $tableName, &$param) {
			...
			if ($col['key'] === 'PRI' && strpos($col['extra'], 'auto') !== false) {
				...
				foreach ($param as $k => $item) {
					if (isset($item[$col['columnName']])) {	unset($param[$k][$col['columnName']]);}
				}
...
$temp = ob_get_clean();
		...
		if (count($param) > 0) {
			...
			if ($change) {
				foreach ($curTable as $col) {
					if ($col['key'] === 'PRI') {
						$idColName = $col['columnName']; break;
					}
				}
			}
  ...
  public function checkUserHash($session) {
	 ...
	 */
	public function getUserSetting(/*currenUserID/NAME*/) {
		$result = self::getCol('SELECT customization from users WHERE login = ?', ['admin']);
		return count($result) ? json_decode($result[0]) : new \stdClass();
	}
