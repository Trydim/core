...
class XLSXWriter_BufferWriter {
  ...
  protected mixed $check_utf8 = false;

  public function __construct($filename, $fd_open_flags = 'w', $check_utf8 = false) {
    $this->check_utf8 = $check_utf8;
    $this->fd = fopen($filename, $fd_open_flags);
    if ($this->fd === false) {
...
class XLSXWriter {
  ...
  public function setKeywords(array $keywords = ['']): void { $this->keywords = $keywords; }
  ...
  public function writeToString(): false|string
  {
    $temp_file = $this->tempFilename();
    self::writeToFile($temp_file);
  ...
  protected function initializeSheet(string $sheet_name, array $col_widths = [], bool $auto_filter = false, bool $freeze_rows = false, bool $freeze_columns = false): void
  {
    ...
    $this->sheets[$sheet_name] = (object)array(
      ...
      'row_count'          => 0,
      'file_writer'        => new XLSXWriter_BufferWriter($sheet_filename),
      'columns'            => [],
      'merge_cells'        => [],
  ...
  private function addCellStyle($number_format, string $cell_style_string): false|int|string
  {
  ...
  private function initializeColumnTypes(array $header_types): array
  {
    $column_types = [];
  ...
  public function writeSheetRow(string $sheet_name, array $row, $row_options = null): void
  {
    if (empty($sheet_name))
      return;
  ...
  protected function finalizeSheet(string $sheet_name): void
  {
  ...
  public function markMergedCell(string $sheet_name, int $start_cell_row, int $start_cell_column, int $end_cell_row, int $end_cell_column): void
  {
    if (empty($sheet_name) || $this->sheets[$sheet_name]->finalized)
      return;
  ...
  protected function writeCell(XLSXWriter_BufferWriter &$file, int $row_number, int $column_number, $value, $num_format_type, $cell_style_idx): void
  {
  ...
  protected function writeStylesXML(): string
  {
    ...
    $file = new XLSXWriter_BufferWriter($temporary_filename);
  ...
  public function buildSheetRelationshipXML(int $sheetId): string
  {
  ...
  public static function sanitize_filename(string $filename): array|string //http://msdn.microsoft.com/en-us/library/aa365247%28VS.85%29.aspx
  {
  ...
  public static function xmlspecialchars(string $val) {
    ...
    static $goodChars = "                              ";
    return strtr(htmlspecialchars((string)$val, ENT_QUOTES | ENT_XML1 | ENT_SUBSTITUTE), $badChars, $goodChars);//str appears to be faster than str_replace
  }

  public static function array_first_key(array $arr): int|string|null
  {
    reset($arr);
    return key($arr);
  }
  ...
  public static function add_to_list_get_index(&$haystack, $needle): bool|int|string
  {
  ...
  public static function convert_date_time($date_input): float|int|string //thanks to Excel::Writer::XLSX::Worksheet.pm (perl)
  {
    ...
    # days by normalizing the year in relation to the epoch. Thus the year 2000
   ...
   * @throws Exception
   */
  public function addImage(string $imagePath, int $imageId, array $imageOptions = []): void
  {
