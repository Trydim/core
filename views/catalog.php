...
<div id="catalogForm">
  ...
  <div class="container-fluid position-relative">
    ...
    <p-dialog v-model:visible="optionsModal.display" :modal="true" style="min-width: 70vw">
      ...
      <div v-if="queryParam.dbAction !== 'deleteOptions'">
        <div class="row">
          <!-- Основное -->
          <div class="col">
            <!-- Имя -->
            ...
            <div class="p-inputgroup my-2">
              ...
              <span v-if="!optionsModal.single" class="p-inputgroup-addon">
                <p-checkbox v-tooltip.bottom="'Не редактировать'" :binary="true" v-model="fieldChange.moneyInputId"></p-checkbox>
              </span>
            ...
            <div class="p-inputgroup my-2">
              ...
              <template v-if="optionsModal.single">
                <span class="p-inputgroup-addon">Сумма</span>
                <p-input-number v-model="option.outputPrice" :min-fraction-digits="2"></p-input-number>
              </template>
            </div>
            <!-- Доступен -->
            <div class="p-inputgroup my-2">
            ...
            <!-- Показать параметры -->
            <div v-if="!optionsModal.single" class="p-inputgroup my-2">
              <span class="p-inputgroup-addon col-6">Показать параметры</span>
          ...
          <div v-if="optionsModal.single || fieldChange.property" class="col overflow-auto" style="max-height: 90vh">
            ...
            <div v-for="(prop, key, index) of properties" class="p-inputgroup mb-2" :key="'t' + key">
              ...
              <p-input-text v-if="prop.type === 'text'" v-model="option.property[key]"></p-input-text>
              <p-input-number v-else-if="prop.type === 'number'" class="p-inputtext-sm" show-buttons
                              v-model="option.property[key]"
              ></p-input-number>
              <p-textarea v-else-if="prop.type === 'textarea'" v-model="option.property[key]" style="min-height: 42px"></p-textarea>
              <p-calendar v-else-if="prop.type === 'date'" date-format="dd.mm.yy" v-model="option.property[key]"></p-calendar>
    ...
    <p-dialog v-model:visible="optionsModal.chooseFileDisplay" :modal="true">
      <template #header>
        <h4>Библиотека файлов</h4>
      </template>

      <i v-if="filesLoading" class="pi pi-spinner"></i>
      <div v-else class="row">
        <div v-for="file of loadedFiles" class="col-12 col-lg-4 card">
          <label class="w-100" role="button" :for="'files-' + file.id">
            <p-image class="card-img-top img-fluid" preview
                     :src="file.src" :alt="file.name">
          </label>
          <div class="card-body d-flex">
            <div class="form-check mx-auto">
              <input class="form-check-input" type="checkbox" name="filesUpSelected[file.id]" :value="file.id" :id="file.id">
              <label class="form-check-label" :for="'files-' + file.id" role="button">{{ file.name }}</label>
            </div>
          </div>
        </div>
      </div>
      ...
      <template #footer>
        <p-button label="Закрыть" @click="optionsModal.chooseFileDisplay = false"></p-button>
      </template>
    </p-dialog>
